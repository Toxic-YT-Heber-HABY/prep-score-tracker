...
import { ArrowLeft, ArrowRight, ArrowUp, BookOpen, CheckCircle2, Copy, Search } from 'lucide-react';
...
const Guide = () => {
  ...
  const spanishSteps = ["Crear categorías", "Agregar actividades", "Configurar pesos", "Ver resultados"];
  const englishSteps = ["Create categories", "Add activities", "Configure weights", "View results"];
  ...
  const spanishTutorialSteps = [{
    ...
    content: "Comienza creando las categorías para tus calificaciones (exámenes, tareas, etc.) y asignando el porcentaje que cada categoría representa en tu calificación final. Recuerda que la suma debe ser exactamente 100%."
  }, {
    title: "Paso 2: Agregar actividades",
    content: "Dentro de cada categoría, agrega las actividades específicas (por ejemplo: 'Examen parcial 1', 'Tarea semanal', etc.) y asigna el peso que cada actividad tiene dentro de su categoría."
  }, {
    title: "Paso 3: Ingresar calificaciones",
    content: "Ingresa las calificaciones obtenidas en cada actividad en una escala de 0 a 100."
  }, {
    title: "Paso 4: Revisar resultados",
    content: "Revisa la sección de resultados para ver tu calificación final calculada, junto con el desglose por categorías."
  }];
  const englishTutorialSteps = [{
    title: "Step 1: Create categories",
    content: "Start by creating categories for your grades (exams, assignments, etc.) and assigning the percentage that each category represents in your final grade. Remember that the sum must be exactly 100%."
  }, {
    title: "Step 2: Add activities",
    content: "Within each category, add specific activities (e.g., 'Midterm exam 1', 'Weekly assignment', etc.) and assign the weight that each activity has within its category."
  }, {
    title: "Step 3: Enter grades",
    content: "Enter the grades obtained for each activity on a scale of 0 to 100."
  }, {
    title: "Step 4: Review results",
    content: "Check the results section to see your calculated final grade, along with the breakdown by category."
  }];
  ...
  const filterContent = (content: any[], term: string) => {
    ...
    return content.filter(item => {
      if (typeof item === 'string') {
        ...
        // FAQ item
        return item.question.toLowerCase().includes(term.toLowerCase()) || item.answer.toLowerCase().includes(term.toLowerCase());
      } else if (item.title) {
        // Feature or tutorial item
        return item.title.toLowerCase().includes(term.toLowerCase()) || item.description?.toLowerCase().includes(term.toLowerCase()) || item.content?.toLowerCase().includes(term.toLowerCase());
      }
      return false;
    });
  };
  return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <img src="/lovable-uploads/22c442b5-67ed-4e06-a4bc-4be99d33c236.png" alt="HABY Logo" className="h-8 w-auto mr-2" />
              <span className="text-xl font-bold text-education-primary">HABY Score Tracker</span>
            </div>
            <div className="flex items-center gap-4">
              <button className="text-gray-600 dark:text-gray-300 hover:text-education-primary dark:hover:text-education-secondary transition-colors" onClick={() => {/* Toggle theme function would go here */}}>
                {language === 'es' ? 'Cambiar tema' : 'Toggle theme'}
              </button>
              <button className="text-gray-600 dark:text-gray-300 hover:text-education-primary dark:hover:text-education-secondary transition-colors" onClick={() => {/* Toggle language function would go here */}}>
                {language === 'es' ? 'EN' : 'ES'}
              </button>
            </div>
          </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-white/80 max-w-2xl">
                {language === 'es' ? 'Aprende a utilizar todas las funciones de la calculadora de calificaciones HABY para obtener el máximo provecho.' : 'Learn how to use all the features of the HABY grade calculator to get the most out of it.'}
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" className="bg-white/10 hover:bg-white/20 text-white border-white/20" onClick={() => setShowTutorial(!showTutorial)}>
                {showTutorial ? language === 'es' ? 'Cerrar tutorial' : 'Close tutorial' : language === 'es' ? 'Iniciar tutorial' : 'Start tutorial'}
              </Button>
            </div>
          </div>
        </div>
        ...
        {/* Tutorial Overlay */}
        {showTutorial && <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full p-6 shadow-xl">
              ...
              <div className="border-t pt-4 flex justify-between">
                ...
                <div className="flex gap-1">
                  {tutorialSteps.map((_, idx) => <span key={idx} className={`h-2 w-2 rounded-full ${idx === currentStep ? 'bg-education-primary' : 'bg-gray-300'}`} />)}
                </div>
                {currentStep < tutorialSteps.length - 1 ? <Button onClick={() => setCurrentStep(currentStep + 1)}>
                    {language === 'es' ? 'Siguiente' : 'Next'}
                  </Button> : <Button onClick={() => setShowTutorial(false)}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 mb-6 shadow-md">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            <div className="relative w-full md:w-64">
              ...
              <Input placeholder={language === 'es' ? "Buscar en la guía..." : "Search guide..."} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-8" />
            </div>
          </div>
        </div>
          ...
          <div className="md:col-span-1">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-md sticky top-4">
              ...
              <nav className="space-y-1">
                <a href="#overview" className="block py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={e => {
                e.preventDefault();
                scrollToSection('overview');
              }}>
                  {language === 'es' ? 'Vista general' : 'Overview'}
                </a>
                <a href="#features" className="block py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={e => {
                e.preventDefault();
                scrollToSection('features');
              }}>
                  {language === 'es' ? 'Características principales' : 'Main features'}
                </a>
                <a href="#how-to" className="block py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={e => {
                e.preventDefault();
                ...
                <div className="pl-4 border-l border-gray-200 dark:border-gray-700 ml-2 mt-1">
                  {steps.map((step, index) => <a key={index} href={`#step-${index + 1}`} className="block py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={e => {
                  e.preventDefault();
                  scrollToSection(`step-${index + 1}`);
                }}>
                      {step}
                    </a>)}
                </div>
                <a href="#faq" className="block py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={e => {
                e.preventDefault();
                scrollToSection('faq');
              }}>
                  {language === 'es' ? 'Preguntas frecuentes' : 'FAQ'}
                </a>
                <a href="#examples" className="block py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={e => {
                e.preventDefault();
                setActiveTab('examples');
                setTimeout(() => {
                  scrollToSection('examples');
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
              ...
              <TabsContent value="guide" className="space-y-6">
                ...
                <section id="overview" className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                  ...
                  <p className="text-gray-700 dark:text-gray-300">
                    {language === 'es' ? 'HABY Score Tracker es una calculadora de calificaciones que te permite organizar tus evaluaciones por categorías y actividades, asignando pesos específicos para obtener tu calificación final de manera precisa. Con HABY, puedes calcular fácilmente tus calificaciones finales basadas en diferentes criterios de evaluación y ponderaciones.' : 'HABY Score Tracker is a grade calculator that allows you to organize your evaluations by categories and activities, assigning specific weights to obtain your final grade accurately. With HABY, you can easily calculate your final grades based on different evaluation criteria and weightings.'}
                  </p>
                </section>
                ...
                <section id="features" className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filterContent(features, searchTerm).map((feature, index) => <div key={index} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <section id="how-to" className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                  ...
                  {steps.map((step, index) => <div id={`step-${index + 1}`} key={index} className="mb-6 last:mb-0">
                ...
                <section id="faq" className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    {filterContent(faq, searchTerm).map((item, index) => <AccordionItem key={index} value={`faq-${index}`}>
...
- Project (100%): 90`}
                      ...
                      <div className="mt-4">
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {language === 'es' ? 'Calificación final calculada: 88.5 / 100 = 8.85 / 10' : 'Final calculated grade: 88.5 / 100 = 8.85 / 10'}
                        </p>
                        <div className="mt-2">
                    ...
                    <div className="border rounded-lg p-4">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded relative">
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button size="icon" variant="outline" className="absolute top-2 right-2 h-8 w-8" onClick={() => handleCopyExample(language === 'es' ? 'Ejemplo 2 - Categorías:\n\nTeoría (50%)\n- Examen teórico 1 (25%): 78\n- Examen teórico 2 (25%): 82\n- Examen final (50%): 88\n\nLaboratorio (40%)\n- Práctica 1 (20%): 90\n- Práctica 2 (20%): 85\n- Práctica 3 (20%): 92\n- Proyecto de laboratorio (40%): 95\n\nAsistencia (10%)\n- Asistencia (100%): 90' : 'Example 2 - Categories:\n\nTheory (50%)\n- Theory exam 1 (25%): 78\n- Theory exam 2 (25%): 82\n- Final exam (50%): 88\n\nLaboratory (40%)\n- Lab 1 (20%): 90\n- Lab 2 (20%): 85\n- Lab 3 (20%): 92\n- Lab project (40%): 95\n\nAttendance (10%)\n- Attendance (100%): 90')}>
                                <Copy className="h-4 w-4" />
                              </Button>
                            </TooltipTrigger>
                            <TooltipContent>
...
- Attendance (100%): 90`}
                      ...
                      <div className="mt-4">
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {language === 'es' ? 'Calificación final calculada: 87.8 / 100 = 8.78 / 10' : 'Final calculated grade: 87.8 / 100 = 8.78 / 10'}
                        </p>
                        <div className="mt-2">
                          <Button variant="outline" size="sm" onClick={() => {
                          // This would load the example in the calculator
            ...
            <button className="fixed bottom-6 right-6 bg-education-primary text-white rounded-full p-3 shadow-lg hover:bg-education-secondary transition-colors" onClick={() => window.scrollTo({
            top: 0,
            behavior: 'smooth'
          })}>
