
import React from "react";
import { Facebook, Instagram, Youtube, Twitter, MessageSquare, Mail, History } from 'lucide-react';
import { useI18n } from "@/lib/i18n";
import { toast } from "sonner";
import { Link } from "react-router-dom";

/**
 * Footer component with social media links
 */
const Footer = () => {
  const { language } = useI18n();
  
  /**
   * Handle email support click
   */
  const handleEmailSupport = () => {
    window.location.href = "mailto:habyopenthedoors@gmail.com";
    toast.success(language === 'es' 
      ? 'Abriendo cliente de correo electrónico' 
      : 'Opening email client');
  };
  
  return (
    <footer className="bg-white dark:bg-gray-800 py-6 border-t dark:border-gray-700 mt-12 transition-colors duration-300 rounded-t-2xl">
      <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center rounded-2xl">
        <div className="flex items-center mb-4 md:mb-0">
          <Link to="/" className="flex items-center hover:opacity-90 transition-opacity">
            <img 
              src="/lovable-uploads/22c442b5-67ed-4e06-a4bc-4be99d33c236.png" 
              alt="HABY Logo" 
              className="h-8 w-auto mr-2 rounded-lg" 
            />
            <span className="text-education-primary font-semibold dark:text-education-secondary">HABY</span>
          </Link>
        </div>
        <div className="flex flex-col items-center md:items-end">
          <div className="flex gap-4 mb-3">
            <a href="https://www.facebook.com/zadkiel.garcia.31" target="_blank" rel="noopener noreferrer" 
              className="text-education-primary hover:text-education-secondary transition-colors p-2 rounded-full hover:bg-education-light/20" title="Facebook">
              <Facebook size={20} />
            </a>
            <a href="https://www.instagram.com/habydoors/" target="_blank" rel="noopener noreferrer" 
              className="text-education-primary hover:text-education-secondary transition-colors p-2 rounded-full hover:bg-education-light/20" title="Instagram">
              <Instagram size={20} />
            </a>
            <a href="https://www.youtube.com/@HABYOpenDoors?themeRefresh=1" target="_blank" rel="noopener noreferrer" 
              className="text-education-primary hover:text-education-secondary transition-colors p-2 rounded-full hover:bg-education-light/20" title="YouTube">
              <Youtube size={20} />
            </a>
            <a href="https://x.com/Haby_Open_Doors" target="_blank" rel="noopener noreferrer" 
              className="text-education-primary hover:text-education-secondary transition-colors p-2 rounded-full hover:bg-education-light/20" title="Twitter">
              <Twitter size={20} />
            </a>
            <a href="https://wa.me/5256536812377" target="_blank" rel="noopener noreferrer" 
              className="text-education-primary hover:text-education-secondary transition-colors p-2 rounded-full hover:bg-education-light/20" title="WhatsApp">
              <MessageSquare size={20} />
            </a>
            <button onClick={handleEmailSupport} 
              className="text-education-primary hover:text-education-secondary transition-colors p-2 rounded-full hover:bg-education-light/20" title={language === 'es' ? "Soporte técnico" : "Technical support"}>
              <Mail size={20} />
            </button>
          </div>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            &copy; {new Date().getFullYear()} HABY Score Tracker - Desarrollado por <span className="font-medium">Heber Zadkiel García Pérez</span>
          </p>
          <div className="flex flex-wrap gap-3 mt-2 justify-center md:justify-end">
            <Link to="/terms" className="text-xs text-education-primary hover:underline px-2 py-1 rounded-lg hover:bg-education-light/20">
              {language === 'es' ? "Términos y condiciones" : "Terms and conditions"}
            </Link>
            <Link to="/privacy" className="text-xs text-education-primary hover:underline px-2 py-1 rounded-lg hover:bg-education-light/20">
              {language === 'es' ? "Política de privacidad" : "Privacy policy"}
            </Link>
            <Link to="/contact" className="text-xs text-education-primary hover:underline px-2 py-1 rounded-lg hover:bg-education-light/20">
              {language === 'es' ? "Contacto" : "Contact"}
            </Link>
            <Link to="/version-history" className="text-xs text-education-primary hover:underline flex items-center px-2 py-1 rounded-lg hover:bg-education-light/20">
              <History size={12} className="mr-1" />
              {language === 'es' ? "Historial de versiones" : "Version history"}
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
